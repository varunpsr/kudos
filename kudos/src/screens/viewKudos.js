...
class ViewKudos extends React.Component {
    ...
    getKudos() {
        axios({
            url: "http://127.0.0.1:8000/kudos/received",
            method: "get",
            headers: {
            ...
            <div key={index}>
                <CardGroup style={{ paddingTop: 20, paddingLeft: 10, paddingRight: 10, alignContent: 'center' }}>
                    <Card style={{ paddingLeft: 20, paddingRight: 20 }}>
                        <Card.Body>
                            <Card.Title style={{ flexWrap: "wrap", width: 200 }} > From: <b> {item.from_user.full_name} </b> </Card.Title>
...
export default connect(mapStateToProps)(ViewKudos);
